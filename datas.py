...
class SingleTaskDataset(torch.utils.data.Dataset):
    def __init__(self, task_id, 
                 target_encode, claim_encode, label_encode):
        ...
        self.y = torch.FloatTensor([label for label in label_encode])

    def __len__(self):
        return len(self.x1)

    def __getitem__(self, index):
        return self.task_id, self.x1[index], self.x2[index], self.y[index]
    ...
    def collate_fn(batch, pad_token_id):
        ...
        y = torch.FloatTensor([data[3].numpy() for data in batch])
