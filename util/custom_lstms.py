...
'''
Reference: https://github.com/pytorch/pytorch/blob/cbcb2b5ad767622cf5ec04263018609bde3c974a/benchmarks/fastrnns/custom_lstms.py
...
def test_script_stacked_bidir_lnlstm(seq_len, batch, input_size, hidden_size,
    ...
    states = [[LSTMState(torch.randn(batch, hidden_size),
               ...
               for _ in range(2)]
              for _ in range(num_layers)]
    rnn = script_lnlstm(input_size, hidden_size, num_layers, bidirectional=True)

    # just a smoke test
    out, out_state = rnn(inp, states)

    # # another forward
    # inp = torch.randn(seq_len, batch-1, input_size)
    # out, out_state = rnn(inp, out_state)

class LayerNormLSTM(jit.ScriptModule):
    def __init__(self, input_size, hidden_size, num_layers=1,
                 bias=True, batch_size=1, batch_first=False, dropout=0,
                 bidirectional=False):
        super(LayerNormLSTM, self).__init__()

        # the following are not implemented
        if bias is False:
            raise ValueError('bias can not be False')
        elif dropout is True:
            raise ValueError('dropout can only be 0')

        # init hidden and cell state
        self.state = [[LSTMState(torch.randn(batch_size, hidden_size),
                                torch.randn(batch_size, hidden_size))
                       for _ in range(2)]
                      for _ in range(num_layers)]

        # init LayerNormLSTM layer
        self.lnlstm = script_lnlstm(input_size=input_size,
    ...
    def forward(self, batch):
        # swap input dimension if batch_first is True: (B, S, H) -> (S, B, H)
        if self.batch_first is True:
            batch = batch.transpose(0, 1)

        # get hidden state of the last layer
        output, state = self.lnlstm(batch, self.state)
        # self.state = state

        # swap output dimension if batch_first is True: (S, B, H) -> (B, S, H)
        if self.batch_first is True:
...
# test_script_stacked_bidir_lnlstm(5, 32, 3, 7, 4)