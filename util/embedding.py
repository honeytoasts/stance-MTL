...
class BaseEmbedding:
    def __init__(self, embedding='twitter', embedding_dim=200):
    ...
    def load_file_embedding(self, id_to_token):
        word_embeddings = {}
        vectors = []
        ...
        ids = sorted(id_to_token.keys())[1:]  # exclude '[pad]' token
        for idx in ids:
        ...
        assert len(self.vector) == len(id_to_token)

    def load_fasttext_embedding(self, id_to_token):
        vectors = []
        ...
        ids = sorted(id_to_token.keys())[1:]  # exclude '[pad]' token
        for idx in tqdm(ids, desc='load embedding'):
            token = id_to_token[idx]
